...
TEMP_DIR = "temp"
LAME_BINARY = "D:/Install_files/lame3.99.5/lame.exe"
KHZ = 16
SHIFT = 150
...
def parse_args():
    ...
    app.add_argument("--rgb_kt", "-r", default=12, type=int,
                     help="Aberrations power, 16 is default")
    app.add_argument("--lp", type=int, default=25,
                     help="Value in range [0..50]. The bigger, "
                           "the brighter lights are")
    app.add_argument("--rp", type=int, default=75,
                     help="Value in range [50..100]. The lesser, "
                          "the deeper shadows are")
    app.add_argument("--gamma", "-g", type=float, default = 0.1,
...
def process_layer(layer, s_qual, bitrate, ch_num):
    ...
    if rc != 0:
        sys.exit("Sorry, lame failed-1")

    process_mp3(mp3_compressed, layer.shape, argument.ch_num)

    rc = subprocess.call(mp3_decompr, shell=True)
    if rc != 0:
        sys.exit("Sorry, lame failed-2")
